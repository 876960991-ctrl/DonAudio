...
// 根据名字查创作者数据，优先查 DNAuth 注册用户 → creators 字典 → 占位
window.DONAUDIO_DATA.getCreator = function (name) {
  if (!name) return null;
  // 1) 先查 DNAuth 真实注册用户
  try {
    const users = JSON.parse(localStorage.getItem('dnUsers.v1') || '{}');
    if (users[name]) {
      const u = users[name];
      // 用昵称做稳定 hash 算出"假的"粉丝数等（只是为了演示视觉），保持每次刷新一致
      let h = 0;
      for (let i = 0; i < name.length; i++) h = ((h << 5) - h + name.charCodeAt(i)) >>> 0;
      const fakeFollowers = u.followers != null ? u.followers : (1200 + (h % 8000));
      const fakeCoops = u.coops != null ? u.coops : (8 + (h % 60));
      const fakeResp = u.response || ['2h','4h','6h','8h','12h','24h'][h % 6];
      return {
        name: u.name,
        role: u.role || '声音创作者',
        location: u.city || '未知',
        avatar: u.avatar || u.avatarChar || u.name.charAt(0),
        avatarImg: u.avatar && u.avatar.startsWith('data:') ? u.avatar : '',
        joined: new Date(u.createdAt).toISOString().slice(0,7).replace('-','.'),
        bio: u.bio || '该用户还没有完善资料。',
        tags: u.tags || [],
        skills: u.skills || [],
        prefs: u.prefs || '',
        coverImg: u.coverImg || '',
        ...
        info: [
          { k: '所在地', v: u.city || '—' },
          { k: '邮箱', v: u.email || '—' },
          { k: '手机', v: u.phone ? (u.phone.slice(0,3)+'****'+u.phone.slice(7)) : '—' },
          { k: '偏好', v: u.prefs || '—' },
        ],
        works: u.works || [],
        feed: u.feed || [],
...
/* ==================================================================
   模拟注册用户（5 板块 × 10 = 50 人）
   每个用户自动带 2-3 件作品，加入到 works[sec]
   ================================================================== */
(function seedSimUsers() {
  const cities = ['北京','上海','广州','成都','杭州','深圳','武汉','西安','南京','重庆'];
  const statusOptions = [
    { v: '开放中', emoji: '✅' },
    { v: '档期紧', emoji: '⏳' },
    { v: '仅内推', emoji: '🔒' },
  ...
  const bios = {
    arranging:  ['专注华语流行编曲。','独立厂牌编曲合作伙伴。','偏爱民谣与后摇融合的编曲思路。','从民乐出发做现代流行。','做 demo 很快，交付也快。','平时写一些 Lofi 的编曲小样。','主修爵士编配，副业接广告。','编过几十首独立艺人专辑。','擅长弦乐 Layer 与空间安排。','节奏设计与声部分离爱好者。'],
    mixing:     ['主业华语流行混音，副业 Atmos。','棚里十年，能把细节磨到极致。','只做人声，不碰乐器混音。','耳机混音派。','重视 Loudness 与动态的平衡。','独立厂牌长期合作。','从家庭棚起步，现已升级到 7.1.4。','Pop / R&B 人声处理为主。','做过金曲奖提名作品的母带。','习惯用 Waves + 自带插件的极简流派。'],
    songwriting:['Top Line 新人，侧重流行。','民谣词曲作者。','电子情歌专场。','歌词教学 + 约稿两不误。','签约某独立厂牌。','写作方向：都市抒情、失意片刻。','国风作词者，重意象。','Co-Write 爱好者。','Hook 实验作品集持续更新。','Demo Pack 按月更新。'],
    sounddesign:['游戏音效设计，Wwise 熟练。','武器与技能音效设计。','环境音采风爱好者。','UI / 品牌声音识别方向。','Foley 爱好者，录东西有瘾。','独立游戏团队音频一人组。','CG / 过场动画声音设计。','Granular 与 Spectral 合成实验者。','电影后期声效 Layer。','做过几部独立游戏的全套音效。'],
    scoring:    ['电影短片配乐师。','纪录片配乐为主。','广告配乐，短平快风格。','剧集配乐长期合作方。','以管弦为主，电子为辅。','游戏配乐 + 独立乐队双栖。','Cinematic 氛围铺陈老手。','情绪拱形爱好者。','45 秒动机变奏专家。','短视频平台快手配乐。'],
  };
  const tagPool = {
    arranging:  ['流行编曲','国风','电子','管弦','City Pop','民谣','Lofi','Cinematic'],
    mixing:     ['Pop Mix','人声处理','Dolby Atmos','母带','R&B','Rock','电子音乐','Atmos'],
    songwriting:['Topline','国风','抒情','电子情歌','民谣','Hook','Co-Write','概念专辑'],
    sounddesign:['游戏音效','Foley','环境音','UI 声音','武器音效','Granular','Wwise','FMOD'],
    scoring:    ['电影','短片','纪录片','剧集','广告','游戏','管弦','Hybrid'],
  };
  const workTitles = {
    arranging:  ['春日编配 Demo','弦乐小品 No.%','流行编曲示范','City Pop Sketch','Lofi Beat Pack','民谣编配稿','电子国潮小样','管弦乐句探索'],
    mixing:     ['人声 Before/After','母带对比示范','Atmos 演示','鼓组并行压缩','空间混响实验','独立摇滚 Mix','流行 Pop Mix','Vocal Pitch 笔记'],
    songwriting:['Topline Demo #%','情歌小样','失眠二十四小时','都市片段歌词','国风歌词稿','Co-Write 日记','Hook 实验 x%','电子情歌 Demo'],
    sounddesign:['武器击打 Pack','脚步 Foley Pack','UI 声音 Kit','环境音采风','Granular 音色实验','CG 过场音效','独立游戏全套音效','品牌声音识别'],
    scoring:    ['短片主题曲 #%','纪录片配乐片段','广告配乐 30s','剧集主题 Hook','管弦即兴','Cinematic 氛围铺','游戏 CG 配乐','情绪拱形练习'],
  };
  const sectionCn = { arranging:'编曲', mixing:'混音母带', songwriting:'作词作曲', sounddesign:'声音设计', scoring:'影视配乐' };

  window.DONAUDIO_DATA.simUsers = [];

  Object.keys(sectionCn).forEach((sec, secIdx) => {
    const cn = sectionCn[sec];
    for (let i = 1; i <= 10; i++) {
      const name   = `${cn}用户${i}`;
      const city   = cities[(secIdx*3 + i) % cities.length];
      const status = statusOptions[i % 3];
      const tags   = [0,1,2].map(k => tagPool[sec][(i+k) % tagPool[sec].length]);
      const skills = tagPool[sec].slice(0, 4 + (i % 3));
      const followers = 200 + i * 130 + secIdx * 50;
      const worksCount = 2 + (i % 2);  // 每人 2-3 件作品
      const user = {
        name,
        role: cn,
        location: city,
        avatar: name.charAt(0),
        joined: `2025.${String(((i+secIdx) % 12) + 1).padStart(2,'0')}`,
        bio: bios[sec][(i-1) % 10],
        tags,
        skills,
        metrics: {
          works: worksCount,
          followers,
          coops: Math.floor(followers * 0.18),
          response: `${4 + i}h`,
        },
        info: [
          { k: '所在地', v: city },
          { k: '接单状态', v: `${status.emoji} ${status.v}` },
          { k: '擅长', v: tags.slice(0,2).join(' / ') },
          { k: '平均响应', v: `${4 + i} 小时` },
        ],
        works: [],
        feed: [
          { time: `${i} 天前`, body: `刚完成了一个${cn}小项目，${bios[sec][(i-1)%10]}有想合作的朋友欢迎站内信。`, actions: [20+i*3, 5+i, i] }
        ],
        sim: true,      // 标记为模拟用户
        section: sec,
      };

      // 生成该用户的作品，并塞进 works[sec]
      for (let j = 0; j < worksCount; j++) {
        const titleTmpl = workTitles[sec][(i + j) % workTitles[sec].length];
        const t = titleTmpl.replace('%', String(j+1));
        const seed = ((i * 3 + j * 2 + secIdx) % 7) + 1;
        const size = ['sm','md','md','lg','wide'][(i+j) % 5];
        const stats = [
          80 + i*30 + j*17,    // 喜欢
          200 + i*80 + j*45,   // 浏览
          5 + i + j*2,         // 评论
        ];
        const work = { t, a: name, size, stats, seed, _sim: true };
        // 保存到 user.works（供 profile 页展示）
        user.works.push({ t, size, stats, seed });
        // 追加到全局作品流
        if (!window.DONAUDIO_DATA.works[sec]) window.DONAUDIO_DATA.works[sec] = [];
        window.DONAUDIO_DATA.works[sec].push(work);
      }

      // 放进 creators 字典让 profile.html?u=XX 能查到
      window.DONAUDIO_DATA.creators[name] = user;
      window.DONAUDIO_DATA.simUsers.push(user);
    }
  });

  // 把模拟用户的作品也 enrich 音频（前面 enrichWorksWithAudio 只处理了已有的，新追加的要再跑一次）
  const pool = window.DONAUDIO_DATA.audioPool;
  let g = 0;
  Object.keys(window.DONAUDIO_DATA.works).forEach(sec => {
    window.DONAUDIO_DATA.works[sec].forEach((w, i) => {
      if (!w.audio) {
        const idx = (g + (w.seed || 0) + i * 2) % pool.length;
        w.audio = pool[idx].url;
        w.audioMeta = pool[idx];
...
/* ==================================================================
   全站搜索：按关键词匹配 作品 / 用户 / 资讯
   ================================================================== */
window.DONAUDIO_DATA.searchAll = function (q) {
  if (!q || !q.trim()) return { works: [], users: [], news: [], total: 0 };
  const kw = q.trim().toLowerCase();
  const match = (s) => s && String(s).toLowerCase().includes(kw);

  const works = [];
  Object.keys(this.works).forEach(sec => {
    this.works[sec].forEach(w => {
      if (match(w.t) || match(w.a) || match(sec)) {
        works.push({ ...w, _sec: sec });
      }
    });
  });
  ...
  const seen = new Set();
  const users = [];
  Object.values(this.creators).forEach(u => {
    if (seen.has(u.name)) return;
    if (match(u.name) || match(u.role) || match(u.location) || match(u.bio)
        || (u.tags && u.tags.some(match))
        || (u.skills && u.skills.some(match))) {
      users.push(u);
      seen.add(u.name);
    }
  });
  ...
  const news = [];
  Object.keys(this.news).forEach(sec => {
    this.news[sec].forEach(n => {
      if (match(n.title) || match(n.text) || match(n.body) || match(n.tag) || match(n.source)) {
        news.push({ ...n, _sec: sec });
      }
    });
  });

  return { works, users, news, total: works.length + users.length + news.length, keyword: q };
};
